...
- **Full bootstrap (clean install):** `.bin/bootstrap.sh` — installs oh-my-zsh, the dependency toolchain, and runs `install.sh`. Uses `brew bundle` when Homebrew is present; on a **brew/node-less host** (e.g. a restricted shared GPU server) it falls back to installing `rtk` and `agent-browser` directly from their GitHub releases (checksum-verified for `rtk`). Flags: `--skip-browser`, `--force`, `--no-brew`.
