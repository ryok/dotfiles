...
- **CI:** GitHub Actions (`.github/workflows/check.yml`) runs three jobs: `ubuntu` (install/uninstall smoke tests incl. dry-run no-op and idempotency), `lint` (`shellcheck` on both `.bin/*.sh`), and `secrets` (gitleaks scan).
