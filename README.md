...
- **Homebrew も node も無い制約ホスト** (共有 GPU サーバ等): `rtk` / `agent-browser` / `herdr` を GitHub release から直接 DL。rtk は配布された `checksums.txt` で検証、herdr は checksums 未配布のため `bootstrap.sh` に SHA-256 をピン留めして検証 (`HERDR_VERSION` 更新時は併せて更新)、agent-browser は現状未検証。
